...
class VideoRecording:
    ...
    def start_video(cls):
        ...
        Log.debug("Starting video recording with command: %s. Host: %s" % (start_command, cls.get_host()))
        send_command("START_VIDEO '%s'" % start_command, host=cls.get_host())

    @classmethod
    def stop_video(cls):
        stop_command = "pkill -INT ffmpeg"
        Log.debug("Stopping video recording by command: %s" % stop_command)
        send_command("STOP_VIDEO '%s'" % stop_command, host=cls.get_host())

    @classmethod
    def get_video(cls, video_file):
        Log.debug("Pulling video file: %s" % video_file)
