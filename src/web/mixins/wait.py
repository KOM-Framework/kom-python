...
class WaitElementMixin(WaitMixin):
    ...
    def visibility_of_element_located(self, wait_time: int=element_load_time, poll_frequency=POLL_FREQUENCY,
        ...
        Log.info('An expectation for checking that an element "%s" is present on the DOM of a page and visible.'
                 % self.name)
        return self.condition(wait_time, visibility_of_element_located(self.locator), poll_frequency, ignored_exceptions)
    ...
    def text_to_be_present_in_element(self, text: str, wait_time: int=element_load_time, poll_frequency=POLL_FREQUENCY,
                                      ignored_exceptions=None):
...
class WaitBrowserMixin(WaitMixin):
    ...
    def new_window_is_opened(self, current_handles: list, wait_time: int=page_load_time, poll_frequency=POLL_FREQUENCY,
                             ignored_exceptions=None):
